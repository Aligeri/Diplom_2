...
import yandexPraktikum.client.Order;
import yandexPraktikum.client.OrderRequest;
import yandexPraktikum.client.User;
import yandexPraktikum.client.UserRequest;
import yandexPraktikum.model.CreateOrderResponse;
    ...
    public void creatingOrderWithAuthorized() {
        ...
        if (availabilityIngredients) {
            if (ingredientsCorrectHash) {
                ...
                Assert.assertEquals(NAME_WAS_NOT_CHANGED, user.name, responseOrder.body().as(CreateOrderResponse.class).getOrder().getOwner().getName());
            } else {
